 ...
 * 
 * It loads gvcf files listed and performs vcf extraction on the samples and
 * chromosomes specified. gvcf file content is stored in hbase table using
 * chr\tsmapleid as the key and the content in a column gvcf of column family
 * gvcfCF as a text string from gvcf file.
 * 
 * @author yxl01
 *
 */
public class VcfLoadExtract implements Serializable {
    ...
    /**
     * modify this method to change bucket of current chr to specific ranges
     * within a chr to achieve more parallelism.
     * 
    ...
    public void loadVcfFile(JavaHBaseContext hbaseContext, JavaSparkContext sc, String sampleID, String filePath)
            throws Exception {
        JavaRDD<String> rdd = sc.textFile(filePath).filter(s -> !s.startsWith("#"))
                .mapToPair(line -> new Tuple2<String, String>(getBucketID(line), line))
                .reduceByKey((x, y) -> x + "\n" + y).map(x -> x._1 + "##" + sampleID + "##" + x._2);
    ...
    // public static class ChrSampleComparator implements Serializable,
    // Comparator <String> {
    // private static final long serialVersionUID= 1L;
    //
    // @Override
    ...
    public void extractVCF(JavaSparkContext sc, Configuration configuration, String[] chrList, String[] sampleIdList,
        ...
        // JavaRDD<String> headerRdd = sc.parallelize(Arrays.asList(new String[]
        // {"##fileformat=VCFv4.1\n##FILTER=<ID=LowQual,Description=\"Low
        // quality\">\n##FORMAT=<ID=AD,Number=.,Type=Integer,Description=\"Allelic
        // depths for the ref and alt alleles in the order
        ...
        // JavaRDD <String> rdd2A =
        // hbaseContext.bulkGet(TableName.valueOf(tableName), 2, rdd, new
        // GetGvcf(), new ResultGvcf())
        // .filter(x -> x!=null)
        // .mapToPair( x -> new Tuple2<String,String>(x.substring(0,
        // x.indexOf("##")).split("\t")[0], x))
        // .reduceByKey((x,y)-> x + "@@" + y)
        // .map(x -> mergeGvcf(x._2.split("@@")));
        // headerRdd.union(rdd2A).saveAsTextFile(outFilePath);
        // System.err.println("Extract A millis: " +
        // (System.currentTimeMillis()-startMillis));

        JavaRDD<String> headerRdd2 = sc.parallelize(Arrays.asList(new String[]{
        ...
        long startMillis = System.currentTimeMillis();
        JavaRDD<String> rdd2B = hbaseContext
                .bulkGet(TableName.valueOf(tableName), 2, rdd, new GetGvcf(), new ResultGvcf()).filter(x -> x != null)
                .mapToPair(x -> new Tuple2<String, String>(x.substring(0, x.indexOf("##")).split("\t")[0], x))
                .groupByKey().map(x -> mergeGvcf(x)).sortBy(new SortVcf(), true, 1);
    ...
    private String mergeGvcf(String[] lineList) throws Exception {
        ...
        while (true) {
            ...
            // prepare next line for samples that have been advanced one line by
            // genVcfLine()
            for (int sIdx = 0; sIdx < sampleNum; sIdx++) {
    ...
    private static int selectSample(List<String[]> sampleGvcfLineList, VcfLine[] sampleIdxList) throws Exception {
        ...
        if (leastVcfLine == null)
            ...
            return leastSampleIdx;
    }

    /**
     * generate vcf line for all samples, keeping in mind that samples may come
     * in in a different order than the sample ids received in the request and
     * (will be) printed to the vcf file in the header. Also, not all samples
     * will have the same set of format fields, so will need to go through all
     * samples to find all format fields and then construct the format field
     * values for each sample accordingly, preferably format fields are printed
     * in the same order that the original samples used.
     * 
     ...
     * @throws Exception
     */
    private void genVcfLine(List<String[]> sampleGvcfLineList, VcfLine[] sampleIdxList, int curSampleIdx,
            VcfLine[] sampleIdxNextList, StringBuilder vcfLineBuilder) throws Exception {
        ...
        for (VcfLine v : sampleIdxList) {
            ...
            for (String key : v.formatFields) {
                if (!formatList.contains(key)) {
                    if (key.equalsIgnoreCase("GT"))
                        formatList.add(0, key);
                    else
                        formatList.add(key);
                    if (fCount++ > 0)
        ...
        vcfLineBuilder.append("\t");
        // write sample fields. samples may be listed in a different order than
        // the VcfExtractSampleIdList
        for (int i = 0; i < sampleNum; i++) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!exit) {
            ...
            if (runInCluster) {
                sconf = new SparkConf().setAppName("gvcf Load/vcf extract");
                // sconf.set("spark.driver.host", "127.0.0.1");
            } else {
                sconf = new SparkConf().setMaster("local").setAppName("gvcf Load/vcf extract");
                sconf.set("spark.driver.host", "127.0.0.1");
            }
            ...
            try {
                if (cmd.equalsIgnoreCase("load")) {
                    ...
                    throw new Exception("Invalid cmd " + cmd + ", valid cmd: load, extract");
            } finally {
                sc.close();
                sc.stop();
                hconnect.close();
    ...
    private void loadVcfFiles(JavaHBaseContext hbaseContext, JavaSparkContext sc, String files) throws Exception {
        ...
        int failedCount = 0;
        logger.info("starting to load these gvcf files: " + files);
        ...
        if (failedCount > 0) {
            throw new Exception(
                    "Failed vcf load on " + failedCount + " out of total of " + fileList.length + " vcf files");
        }
    }

    private void extractIntoVcfFile(JavaSparkContext sc, Configuration configuration, String sampleIds, String outFile)
            throws Exception {
